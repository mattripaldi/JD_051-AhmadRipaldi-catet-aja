...
export default function OutcomeDelete({ transaction }) {
    const { delete: destroy, processing } = useForm();
    const { success } = useToastContext();

    const handleDelete = (close) => {
        destroy(route('outcome.destroy', [transaction.account_id, transaction.id]), {
            onSuccess: () => {
                success('Transaksi pengeluaran berhasil dihapus!');
                close();
            },
            onError: () => {
